            ...
            <li class="nav-item">
              <div class="dropdown">
                ...
                >
                  ...
                  <span class="badge badge-danger" v-if="favList">{{ favList.length }}</span>
                <span class="badge badge-danger" v-else>0</span>
                </a>
                <div class="dropdown-menu" aria-labelledby="fav">
               <div class="table-responsive p-3 font-size" v-if="favList.length">
                 <h6>收藏清單</h6>
            <table class="table table-hover width ">
              <tbody>
                <tr v-for ="(item, index) in favList" :key="item.id">
...
export default {
  ...
  methods: {
    ...
    deleteFavItem(item, index) {
      ...
      this.$emit('deleteFav', this.favList);
    },
    addFavtoCart(id, qty) {
      console.log(id, qty);
      this.$emit('addFavtoCart', id, qty);
    },
  },
  created() {
    const vm = this;
